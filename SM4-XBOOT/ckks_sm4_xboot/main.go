...
func main() {
	...
	fmt.Printf("[SM4][Config] logN=%d short=%v mod2=xboot-stc roundSchedule=one-boot fusion=sbox-l-fused-optimized tmpBound=0..124 finalKeystreamBound=0..41 finalCTRBound=0..42\n",
		LogN, *flagShort)
	...
	profile := xbootparams.Default(LogN)
	...
	// LogN=12 is an experimental development profile. Do not present this
	// parameter set as a 128-bit security profile.
