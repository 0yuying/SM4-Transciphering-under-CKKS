...
const (
	...
	sm4XBOOTTmpBound            = 124
	sm4XBOOTFinalKeystreamBound = 41
	sm4XBOOTFinalCTRBound       = 42
	sm4SboxLFusedOptimizedMode  = "sbox-l-fused-optimized"
)
...
func (sm4 *SM4Ctr) bootstrapBitsXBOOT(cts []*rlwe.Ciphertext, roundNo int, stage string) {
	if len(cts) == 0 {
		return
	}

	pairs := (len(cts) + 1) / 2
	workers := sm4.bootstrapWorkerCount(pairs)
	...
	for w := 0; w < workers; w++ {
		...
		go func(eval *bootstrapping.Evaluator) {
			defer wg.Done()
			for {
				pair := int(atomic.AddInt64(&idx, 1) - 1)
				if pair >= pairs {
					return
				}
				i := pair * 2
				if i+1 == len(cts) {
					...
					if err != nil {
						once.Do(func() {
							firstErr = fmt.Errorf("SM4 RoundFunction(round=%d): %s XBOOT bit[%d] failed: %w", roundNo, stage, i, err)
						})
						continue
					}
					cts[i] = out
				...
				out0, out1, err := eval.BootstrapCmplxThenDivide(cts[i], cts[i+1])
				if err != nil {
					once.Do(func() {
						firstErr = fmt.Errorf("SM4 RoundFunction(round=%d): %s XBOOT bits[%d:%d] failed: %w", roundNo, stage, i, i+2, err)
					})
					continue
				}
				cts[i], cts[i+1] = out0, out1
			}
		}(eval)
	}
	wg.Wait()
...
func (sm4 *SM4Ctr) heDecryptPrepared(ciphertexts []uint8, bits int, useCipherXOR bool, numBlock int, keyScheduleDur time.Duration, memStart runtime.MemStats) ([]*rlwe.Ciphertext, error) {
	...
	// CTR plaintext = ciphertext XOR keystream. Keep this additive so the final
	// XBOOT boundary performs the only parity recovery.
	startOutXOR := time.Now()
	if useCipherXOR {
		eval := sm4.Evaluator.ShallowCopy()
		for i := 0; i < outputBits; i++ {
			out[i] = FXORNew(eval, out[i], sm4.encodeCipher[i])
		}
	}
	...
	if finalBootBits > 0 {
		sm4.bootstrapBitsXBOOT(out[:finalBootBits], sm4.rounds+1, "final")
	}
	out = out[:outputBits]
...
func (sm4 *SM4Ctr) RoundFunction(state []*rlwe.Ciphertext, rk []*rlwe.Ciphertext, roundNo int) (rt roundTiming) {
	...
	if roundNo == 1 {
		sm4.logf(true, "[SM4][Round %02d] mod2 strategy=xboot-stc schedule=one-boot fusion=%s tmpBound=0..%d finalKeystreamBound=0..%d finalCTRBound=0..%d (LogN=%d)\n",
			roundNo, sm4SboxLFusedOptimizedMode, sm4XBOOTTmpBound, sm4XBOOTFinalKeystreamBound, sm4XBOOTFinalCTRBound, sm4.params.LogN())
	}
	...
	stageStart = time.Now()
	sm4.bootstrapBitsXBOOT(tmp, roundNo, "tmp")
	...
	// The optimized S-box returns integer parity carriers, not freshly reduced bits.
	// SM4's L-layer is linear, so we intentionally fuse those carriers directly
	// into the ROTL/XOR aggregation and defer parity recovery to the next XBOOT boundary.
	...
	// x4 = x0 + t. The next round's tmp XBOOT, or the final output XBOOT,
	// recovers its parity.
	stageStart = time.Now()
	...
	// === No x4 bootstrap: keep the integer parity carrier across rounds ===
	stageStart = time.Now()
