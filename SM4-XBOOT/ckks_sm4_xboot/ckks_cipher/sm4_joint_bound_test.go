...
type sm4JointOptimizedSboxBounds struct {
	min    [8]int
	max    [8]int
	maxAbs [8]int
}

func TestSM4JointOptimizedScheduleIntegerBounds(t *testing.T) {
	bounds := sm4ComputeJointOptimizedSboxBounds(t)
	...
	t.Logf("joint optimized S-box carrier bounds: min=%v max=%v maxAbs=%v", bounds.min, bounds.max, bounds.maxAbs)
}
...
func TestSM4SboxOutputToLLayerCarrierBounds(t *testing.T) {
	...
	t.Logf("S-box -> L-layer fused-optimized carrier bounds: maxSboxAbs=%d globalL=%d globalXi=%d rotationAwareL=%d rotationAwareXi=%d",
...
func TestSM4OneBootScheduleCarrierBounds(t *testing.T) {
	...
	for round := 0; round < 32; round++ {
		...
		tmpBounds[round] = tmp
		if tmp > sm4XBOOTTmpBound {
			t.Fatalf("round %d tmp carrier bound exceeds XBOOT capacity target: got=%d max=%d", round+1, tmp, sm4XBOOTTmpBound)
		}
		if tmp > maxTmp {
			maxTmp = tmp
		}
	...
	for wordIdx, bound := range maxWord[32:36] {
		if bound > sm4XBOOTFinalKeystreamBound {
			t.Fatalf("final word %d carrier bound exceeds final XBOOT keystream target: got=%d max=%d", wordIdx, bound, sm4XBOOTFinalKeystreamBound)
		}
		if bound > finalMax {
			finalMax = bound
		}
	...
	if maxTmp != sm4XBOOTTmpBound {
		t.Fatalf("one-boot tmp carrier bound changed: got=%d want=%d", maxTmp, sm4XBOOTTmpBound)
	}
	if finalMax != sm4XBOOTFinalKeystreamBound {
		t.Fatalf("one-boot final carrier bound changed: got=%d want=%d", finalMax, sm4XBOOTFinalKeystreamBound)
	}
	if finalMax+1 != sm4XBOOTFinalCTRBound {
		t.Fatalf("final CTR carrier bound changed: got=%d want=%d", finalMax+1, sm4XBOOTFinalCTRBound)
	}

	t.Logf("one-boot XBOOT schedule carrier bounds: tmp=0..%d finalKeystream=0..%d finalCTR=0..%d tmpBounds=%v wordBounds=%v",
		maxTmp, finalMax, finalMax+1, tmpBounds, maxWord[32:36])
}

func sm4ComputeJointOptimizedSboxBounds(t *testing.T) sm4JointOptimizedSboxBounds {
	t.Helper()
	...
	bounds := sm4JointOptimizedSboxBounds{}
	for bit := 0; bit < 8; bit++ {
