...
func (eval Evaluator) bootstrapCmplxThenDivideWithEvalModMode(ctIn0, ctIn1 *rlwe.Ciphertext, lazyMod2 bool) (*rlwe.Ciphertext, *rlwe.Ciphertext, error) {
	...
	if err := eval.Evaluator.Add(ct0, ct1, ct0); err != nil {
		return nil, nil, fmt.Errorf("cannot BootstrapCmplxThenDivide: combine real/imag: %w", err)
	}

	if ct0.Level() < eval.SlotsToCoeffsParameters.LevelQ {
		return nil, nil, fmt.Errorf("cannot BootstrapCmplxThenDivide: level=%d < SlotsToCoeffs.LevelQ=%d", ct0.Level(), eval.SlotsToCoeffsParameters.LevelQ)
	}
	for ct0.Level() > eval.SlotsToCoeffsParameters.LevelQ {
		eval.DropLevel(ct0, 1)
	}
	...
	var err error
	if ct0, err = eval.SlotsToCoeffs(ct0, nil); err != nil {
		return nil, nil, fmt.Errorf("cannot BootstrapCmplxThenDivide: SlotsToCoeffs: %w", err)
	}
	if ct0, _, err = eval.ScaleDown(ct0); err != nil {
		return nil, nil, fmt.Errorf("cannot BootstrapCmplxThenDivide: ScaleDown: %w", err)
	}
	if ct0, err = eval.ModUp(ct0); err != nil {
		return nil, nil, fmt.Errorf("cannot BootstrapCmplxThenDivide: ModUp: %w", err)
	}

	var real, imag *rlwe.Ciphertext
	if real, imag, err = eval.CoeffsToSlots(ct0); err != nil {
