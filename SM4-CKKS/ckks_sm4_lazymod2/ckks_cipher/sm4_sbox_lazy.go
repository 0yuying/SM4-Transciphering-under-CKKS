...
type sm4LazyBucketKey struct {
	level        int
	logScaleBits uint64
}

type sm4LazyBucket struct {
	perBit [8]*rlwe.Ciphertext
}

func (sm4 *SM4Ctr) applySboxStage(tmp []*rlwe.Ciphertext) (sm4SboxOpStats, error) {
	return sm4.applySboxStageLazy(tmp)
}

func (sm4 *SM4Ctr) applySboxStageLazy(tmp []*rlwe.Ciphertext) (sm4SboxOpStats, error) {
	sboxTasks := 4
	...
	if sboxWorkers <= 1 {
		...
		for byteIdx := 0; byteIdx < sboxTasks; byteIdx++ {
			out, stats, err := sm4.sm4SubbyteLUTLazy(evalCopy, tmp[byteIdx*8:(byteIdx+1)*8])
			if err != nil {
				return sm4SboxOpStats{}, fmt.Errorf("lazy SBox byte %d failed: %w", byteIdx, err)
			}
			byteOut[byteIdx] = out
		...
		for w := 0; w < sboxWorkers; w++ {
			...
			go func(evalCopy *bootstrapping.Evaluator) {
				...
				for {
					...
					if err != nil {
						once.Do(func() {
							firstErr = fmt.Errorf("lazy SBox byte %d failed: %w", byteIdx, err)
						})
						continue
					}
					byteOut[byteIdx] = out
	...
	for byteIdx := 0; byteIdx < sboxTasks; byteIdx++ {
		for bit := 0; bit < 8; bit++ {
			if byteOut[byteIdx][bit] == nil {
				return sm4SboxOpStats{}, fmt.Errorf("lazy SBox byte %d bit %d has nil output", byteIdx, bit)
			}
...
func (sm4 *SM4Ctr) sm4SubbyteLUTLazy(eval *bootstrapping.Evaluator, sboxIn []*rlwe.Ciphertext) (out [8]*rlwe.Ciphertext, stats sm4SboxOpStats, err error) {
	if len(sboxIn) != 8 {
	...
	buckets := make(map[sm4LazyBucketKey]*sm4LazyBucket)
	bucketKeys := make([]sm4LazyBucketKey, 0, 8)
	...
	for i := 0; i < len(left); i++ {
		for j := 0; j < len(right); j++ {
			...
			if err != nil {
				return out, stats, fmt.Errorf("lazy cross product(%d,%d) failed: %w", i, j, err)
			}

			key := sm4LazyBucketKey{
				level:        prod.Level(),
				logScaleBits: sm4QuantizedLogScaleBitsHalf(prod.LogScale()),
			}
			bucket := buckets[key]
			if bucket == nil {
				bucket = &sm4LazyBucket{}
				buckets[key] = bucket
