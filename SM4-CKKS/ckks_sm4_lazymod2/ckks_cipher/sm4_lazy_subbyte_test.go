...
func lazyMonomialMasksLayout() []uint64 {
...
func buildLazyLeftRight(t *testing.T, sm4 *SM4Ctr, eval *bootstrapping.Evaluator, sboxIn []*rlwe.Ciphertext) (left, right []*rlwe.Ciphertext) {
	t.Helper()
	if len(sboxIn) != 8 {
...
func TestSM4LazyMonomialLayoutMatchesLayeredCombineBin(t *testing.T) {
	ref := sm4MonomialMasks(t)
	got := lazyMonomialMasksLayout()
...
func TestSM4LazyLevelNormalizationToMinLevel(t *testing.T) {
	tc := newMod2TestContext(t)
	sm4 := newSM4SboxHarness(t, tc.eval)
	eval := tc.eval.ShallowCopy()

	left, right := buildLazyLeftRight(t, sm4, eval, cloneBits(makeByteInput(t, tc, 0x6d)))
...
func TestSM4LazyLogScaleQuantizationHalfStep(t *testing.T) {
...
func TestSM4LazyQuantizedBucketCountNotMoreThanExact(t *testing.T) {
	tc := newMod2TestContext(t)
	sm4 := newSM4SboxHarness(t, tc.eval)
	eval := tc.eval.ShallowCopy()

	left, right := buildLazyLeftRight(t, sm4, eval, cloneBits(makeByteInput(t, tc, 0x53)))
	if _, err := sm4.normalizeToTargetLevel(eval, left, right); err != nil {
		t.Fatalf("normalizeToTargetLevel failed: %v", err)
	}

	exact := map[sm4LazyBucketKey]struct{}{}
	quant := map[sm4LazyBucketKey]struct{}{}
	...
	for i := 0; i < len(left); i++ {
		for j := 0; j < len(right); j++ {
			...
			exact[sm4LazyBucketKey{level: prod.Level(), logScaleBits: math.Float64bits(prod.LogScale())}] = struct{}{}
			quant[sm4LazyBucketKey{level: prod.Level(), logScaleBits: sm4QuantizedLogScaleBitsHalf(prod.LogScale())}] = struct{}{}
		}
	}
...
func TestSM4LazySubByteCounterAndOutputAgreement(t *testing.T) {
	tc := newMod2TestContext(t)
	sm4 := newSM4SboxHarness(t, tc.eval)
	...
	lazyOutArr, stats, err := sm4.sm4SubbyteLUTLazy(tc.eval.ShallowCopy(), cloneBits(in))
	if err != nil {
		t.Fatalf("lazy subbyte failed: %v", err)
	}
	lazyOut := make([]*rlwe.Ciphertext, 8)
	for bit := 0; bit < 8; bit++ {
		lazyOut[bit] = lazyOutArr[bit]
	}

	_, eagerValues := decodeByteFromBits(t, tc, eagerIn)
	_, lazyValues := decodeByteFromBits(t, tc, lazyOut)

	for bit := 0; bit < 8; bit++ {
		if math.IsNaN(eagerValues[bit]) || math.IsInf(eagerValues[bit], 0) {
			t.Fatalf("eager output invalid at bit %d: %.6f", bit, eagerValues[bit])
		}
		if math.IsNaN(lazyValues[bit]) || math.IsInf(lazyValues[bit], 0) {
			t.Fatalf("lazy output invalid at bit %d: %.6f", bit, lazyValues[bit])
		}
	}

	if stats.relin >= sm4SboxEagerRelinPerByte {
		t.Fatalf("lazy relin count not reduced: got %d, baseline %d", stats.relin, sm4SboxEagerRelinPerByte)
	}
	if stats.rescale >= sm4SboxEagerRescalePerByte {
		t.Fatalf("lazy rescale count not reduced: got %d, baseline %d", stats.rescale, sm4SboxEagerRescalePerByte)
	}
