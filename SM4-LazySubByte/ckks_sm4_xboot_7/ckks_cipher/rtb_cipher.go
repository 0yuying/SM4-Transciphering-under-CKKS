...
func XOR(eval *bootstrapping.Evaluator, ct0, ct1 *rlwe.Ciphertext, ctOut *rlwe.Ciphertext) {
	// (x-y)^2
	eval.Sub(ct0, ct1, ctOut)
	eval.MulRelin(ctOut, ctOut, ctOut)
	eval.Rescale(ctOut, ctOut)
}

// //boolean function constructed from Heaan using CKKS scheme
func XORNew(eval *bootstrapping.Evaluator, ct0, ct1 *rlwe.Ciphertext) (ctOut *rlwe.Ciphertext) {
	// (x-y)^2
	ctOut, err := eval.SubNew(ct0, ct1)
	if err != nil {
		panic(err)
	}
	eval.MulRelin(ctOut, ctOut, ctOut)
...
// FXOR uses additive aggregation over Z and relies on an explicit mod2 reduction stage.
func FXOR(eval *bootstrapping.Evaluator, ct0, ct1 *rlwe.Ciphertext, ctOut *rlwe.Ciphertext) {
	eval.Add(ct0, ct1, ctOut)
}

// FXORNew uses additive aggregation over Z and relies on an explicit mod2 reduction stage.
...
func alignLevelCopy(eval *bootstrapping.Evaluator, ct *rlwe.Ciphertext, target int) *rlwe.Ciphertext {
	out := ct.CopyNew()
	if out.Level() > target {
		eval.DropLevel(out, out.Level()-target)
	}
	return out
}

func alignLevelAndScaleCopy(eval *bootstrapping.Evaluator, ct *rlwe.Ciphertext, targetLevel int, targetScale rlwe.Scale) *rlwe.Ciphertext {
	out := alignLevelCopy(eval, ct, targetLevel)
	eval.SetScale(out, targetScale)
	return out
}
...
func minScale(cts ...*rlwe.Ciphertext) rlwe.Scale {
	if len(cts) == 0 {
		return rlwe.NewScale(1)
	}
	scale := cts[0].Scale
	for i := 1; i < len(cts); i++ {
		if cts[i].Scale.Cmp(scale) == -1 {
			scale = cts[i].Scale
		}
	}
	return scale
...
// Mod2Reduce2New reduces a ciphertext encoding values in {0,1,2} to x mod 2 with p2(x)=2x-x^2.
func Mod2Reduce2New(eval *bootstrapping.Evaluator, ct *rlwe.Ciphertext) (ctOut *rlwe.Ciphertext) {
	twoMinusX := ct.CopyNew()
	eval.Mul(twoMinusX, -1.0, twoMinusX)
	eval.Add(twoMinusX, 2.0, twoMinusX)

	ctOut, err := eval.MulRelinNew(ct, twoMinusX)
	if err != nil {
		panic(err)
	}
...
// Mod2Reduce4New reduces a ciphertext encoding values in {0,1,2,3,4} to x mod 2 with:
// p4(x) = (16x - 20x^2 + 8x^3 - x^4)/3.
func Mod2Reduce4New(eval *bootstrapping.Evaluator, ct *rlwe.Ciphertext) (ctOut *rlwe.Ciphertext) {
	// Horner form with integer coefficients first:
	// q4(x) = x * (x * (x * (-x + 8) - 20) + 16)
	// p4(x) = q4(x) / 3
	h := ct.CopyNew()
	eval.Mul(h, -1.0, h)
	eval.Add(h, 8.0, h)

	ctOut, err := eval.MulRelinNew(h, ct)
	if err != nil {
		panic(err)
	}
	if err = eval.Rescale(ctOut, ctOut); err != nil {
	...
	eval.Add(ctOut, -20.0, ctOut)

	ctOut, err = eval.MulRelinNew(ctOut, ct)
	if err != nil {
		panic(err)
	}
	if err = eval.Rescale(ctOut, ctOut); err != nil {
...
// Mod2Reduce6New reduces a ciphertext encoding values in {0,1,2,3,4,5,6} to x mod 2 with:
// p6(x) = (-2x^6 + 36x^5 - 245x^4 + 780x^3 - 1148x^2 + 624x) / 45.
func Mod2Reduce6New(eval *bootstrapping.Evaluator, ct *rlwe.Ciphertext) (ctOut *rlwe.Ciphertext) {
	mulRelinRescale := func(a, b *rlwe.Ciphertext) *rlwe.Ciphertext {
		targetLevel := minLevel(a, b)
		aa := alignLevelCopy(eval, a, targetLevel)
		bb := alignLevelCopy(eval, b, targetLevel)

		out, err := eval.MulRelinNew(aa, bb)
		if err != nil {
			panic(err)
		}
	...
	x1 := ct.CopyNew()
	x2 := mulRelinRescale(x1, x1)
	x3 := mulRelinRescale(x2, x1)
	x4 := mulRelinRescale(x2, x2)
	x5 := mulRelinRescale(x4, x1)
	x6 := mulRelinRescale(x3, x3)

	targetLevel := minLevel(x1, x2, x3, x4, x5, x6)
	x1 = alignLevelCopy(eval, x1, targetLevel)
	x2 = alignLevelCopy(eval, x2, targetLevel)
	x3 = alignLevelCopy(eval, x3, targetLevel)
	x4 = alignLevelCopy(eval, x4, targetLevel)
	x5 = alignLevelCopy(eval, x5, targetLevel)
	x6 = alignLevelCopy(eval, x6, targetLevel)

	ctOut = x6.CopyNew()
	eval.Mul(ctOut, -2, ctOut)
	...
	tmp = x4.CopyNew()
	eval.Mul(tmp, -245, tmp)
	eval.Add(ctOut, tmp, ctOut)

	tmp = x3.CopyNew()
	eval.Mul(tmp, 780, tmp)
	eval.Add(ctOut, tmp, ctOut)

	tmp = x2.CopyNew()
	eval.Mul(tmp, -1148, tmp)
	eval.Add(ctOut, tmp, ctOut)

	tmp = x1.CopyNew()
	eval.Mul(tmp, 624, tmp)
	eval.Add(ctOut, tmp, ctOut)

	eval.Mul(ctOut, 1.0/45.0, ctOut)
	if err := eval.Rescale(ctOut, ctOut); err != nil {
		panic(err)
	}
