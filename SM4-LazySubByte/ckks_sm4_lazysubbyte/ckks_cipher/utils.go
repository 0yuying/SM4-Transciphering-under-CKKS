...
// SetBytesLE fills BitSet from a little-endian byte slice.
...
func ctr(iv *BitSet, ctr uint64) *BitSet {
	...
	// 128-bit little-endian increment: out = iv + ctr.
	carry := uint8(0)
	limit := out.size
	if limit > 64 {
		limit = 64
	...
	for i := 0; i < limit; i++ {
		addBit := uint8((ctr >> uint(i)) & 1)
		sum := out.bits[i] + addBit + carry
		out.bits[i] = sum & 1
		carry = (sum >> 1) & 1
	}

	for i := limit; i < out.size && carry != 0; i++ {
		sum := out.bits[i] + carry
		out.bits[i] = sum & 1
		carry = (sum >> 1) & 1
	}
