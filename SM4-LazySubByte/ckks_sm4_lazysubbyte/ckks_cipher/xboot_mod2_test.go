...
func TestMod2Reduce6NewTruthTable(t *testing.T) {
	tc := newMod2TestContext(t)
	cases := []struct {
		in   float64
		want float64
	...
	for _, c := range cases {
		ct := tc.encryptConst(t, c.in)
		out := Mod2Reduce6New(tc.eval, ct)
		got := tc.decodeSlot0(t, out)
		if math.Abs(got-c.want) > 0.50 {
			t.Fatalf("Mod2Reduce6New(%v): got %.6f, want %.1f", c.in, got, c.want)
		}
	}
}
